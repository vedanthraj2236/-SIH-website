...
export default function Home(){ const { t, active } = useLanguage(); return <main className="home-page">
 <section className="relative overflow-hidden home-dark-hero"><div className="absolute inset-0 heritage-grid opacity-30"/><div className="relative mx-auto max-w-7xl px-4 pb-8 pt-8 text-center md:px-8 md:pb-12 md:pt-10"><div className="mx-auto max-w-3xl animate-fadeUp"><div className="inline-flex items-center gap-2 rounded-full border border-maroon/10 bg-white/75 px-4 py-2 text-[10px] font-bold uppercase tracking-[.22em] text-maroon"><Sparkles size={13}/> India as a living cultural atlas</div><h1 className="mt-4 font-display text-4xl leading-[1] text-ink sm:text-5xl lg:text-6xl">Explore India.<br/><span className="text-maroon">Discover its heritage.</span></h1><p className="mx-auto mt-4 max-w-xl text-sm leading-6 text-ink/55 md:text-base">Map → State → District → Tradition.</p><div className="mx-auto mt-5 max-w-xl"><SearchBar/></div></div><div className="mt-7 text-left home-atlas-frame rounded-[2rem] atlas-dark overflow-hidden"><HeritageAtlas standalone/></div></div></section>
