    ...
    <div className="mx-auto max-w-7xl px-4 py-12 md:px-8 md:py-16">
      ...
      <div className={`mt-2 grid gap-5 ${standalone && level === 'india' ? 'lg:grid-cols-1' : 'lg:grid-cols-[1.65fr_.8fr]'}`}>
        <div ref={mapFrameRef} className="relative min-h-[640px] overflow-hidden rounded-[2rem] border border-maroon/10 bg-[#EEF4F3] shadow-warm">
          ...
          ) : selectedState?.slug === 'karnataka' ? (
            <div className="relative flex h-[640px] w-full items-center justify-center overflow-hidden px-6 py-8">
              <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(229,138,36,.12),transparent_55%)]" />
              <div className="relative z-10 flex w-full max-w-[520px] flex-col items-center">
                <div className="mb-4 text-center">
                  <div className="text-[10px] font-bold uppercase tracking-[.24em] text-maroon/55">Karnataka districts</div>
                  <div className="mt-1 font-display text-2xl text-maroon">Explore Karnataka by district</div>
                </div>
                <div className="relative w-full rounded-[2rem] border border-maroon/10 bg-white/55 p-5 shadow-warm backdrop-blur">
                  <KarnatakaMap
                    onClick={(code) => {
                    ...
                    strokeWidth="1.2"
                    hoverColor="#263A73"
                    className="mx-auto block h-auto max-h-[500px] w-full"
                  />
                </div>
                {selectedDistrict && (
                  <div className="mt-4 rounded-full border border-gold/30 bg-[#263A73] px-4 py-2 text-xs font-semibold text-cream shadow-lg">Selected district: {selectedDistrict}</div>
                )}
              </div>
            </div>
          ) : (
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-4">
        {[['12','curated tradition profiles'],['10','featured state layers'],['30','curated district hubs'],['14','cultural lenses']].map(([value,label])=><div key={label} className="rounded-2xl border border-maroon/10 bg-white/70 p-4"><div className="font-display text-3xl text-maroon">{value}</div><div className="mt-1 text-xs text-ink/50">{label}</div></div>)}
      </div>
    </div>
  </section>
}
