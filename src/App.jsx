...
function Header(){
 ...
 const { t } = useLanguage()
 return <header className="site-header sticky top-0 z-[100] border-b border-maroon/10 bg-cream/88 shadow-sm backdrop-blur-xl">
  <div className="mx-auto flex min-h-[72px] max-w-7xl items-center justify-between gap-4 px-4 py-3 md:px-8">
   ...
   <button className="rounded-full border border-maroon/10 bg-white/85 p-2 text-maroon shadow-sm transition hover:bg-white md:hidden" onClick={()=>setOpen(v=>!v)} aria-label="Toggle navigation">{open?<X/>:<Menu/>}</button>
  </div>
  {open&&<div className="mobile-nav-panel border-t border-maroon/10 bg-cream/98 px-4 pb-5 pt-3 shadow-2xl md:hidden">
